#!/usr/bin/env python3
"""Meeting bot v1b: v1 that worked + faster TTS + transcript logging."""
...
def speak(text):
    ...
    if proc.returncode != 0:
        print(f"[tts err] {proc.stderr.decode()[:100]}", flush=True)
        return
...
async def main():
    ...
    print(f"[loop] Listening... (TTS speed: {TTS_SPEED}x)", flush=True)
